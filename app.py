...
@app.route("/get_months_data")
def get_months_data():
    return json_util.dumps(months_data)
@app.route("/get_years_data")
def get_years_data():
    return json_util.dumps(years_data)
@app.route("/get_dows_data")
def get_dows_data():
    return json_util.dumps(dows_data)
@app.route("/get_origins_data")
def get_origins_data():
    return json_util.dumps(origins_data)
@app.route("/get_dests_data")
def get_dests_data():
    return json_util.dumps(dests_data)
@app.route("/get_carriers_data")
...
#         else :
#             status = 'On Time' 
#         return f'Status: {status}, Prediction: {prediction[0]}'
